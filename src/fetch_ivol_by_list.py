...
def parse_args():
    p = argparse.ArgumentParser(
        description="Fetch IVol options for a list of tickers with delta/DTE filters."
    )
    ...
    p.add_argument(
        ...
        type=int,
        default=31,
        help="Size of date chunks per request (avoid provider limits).",
    )
    p.add_argument(
        "--sleep", type=float, default=0.2, help="Pause between API calls (be polite)."
    )
    p.add_argument(
        "--outdir", default=str(RAW_DIR), help="Output folder (outside repo)."
    )
    p.add_argument(
        "--combine",
        action="store_true",
        help="Also write a combined file across all symbols.",
    )
    p.add_argument(
        "--fmt",
        choices=["parquet", "csv"],
        default="parquet",
        help="Output format per symbol and combined.",
    )
    p.add_argument(
        "--key", default=None, help="IVOL API key (or set IVOL_API_KEY env var)."
    )
    return p.parse_args()
...
def main():
    ...
    print(
        f"Symbols: {len(symbols)} found -> {symbols[:8]}{'...' if len(symbols)>8 else ''}"
    )
        ...
        if frames:
            ...
            if "delta" in out_df.columns:
                ...
                if "call_put" in out_df.columns:
                    ...
                    mask = (
                        (cpcol == "C")
                        & (out_df["delta"].between(abs_lo, abs_hi, inclusive="both"))
                    ) | (
                        (cpcol == "P")
                        & (out_df["delta"].between(-abs_hi, -abs_lo, inclusive="both"))
                    )
                    out_df = out_df[mask]
            # write per-symbol
